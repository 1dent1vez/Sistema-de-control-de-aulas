 ...
 *               2026-05-28 - Validación dinámica de teacher_external_id para administradores.
 */
...
class StoreTeacherAbsenceRequest extends FormRequest
{
    ...
    {
        $rules = [
            'absence_type_id' => ['required', 'integer', 'exists:absence_types,absence_type_id'],
        ...
        // Si el usuario es ADMIN, validar teacher_external_id de forma requerida y existente
        if ($this->user() && $this->user()->role === SamRole::ADMIN) {
            $rules['teacher_external_id'] = ['required', 'string', 'exists:sam_identities,external_id'];
        } else {
            $rules['teacher_external_id'] = ['sometimes', 'string', 'max:50'];
        }
    ...
    public function messages(): array
    {
        return [
            ...
            'teacher_external_id.exists' => 'El docente seleccionado no es valido.',
